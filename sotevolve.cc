...
OXS_EXT_REGISTER(Oxs_SOTEvolve);

/* End includes */

// Constructor
Oxs_SOTEvolve::Oxs_SOTEvolve(
...
{
  // Process arguments
  theta_SH = GetRealInitValue("theta_SH",0.0);
  fm_thickness = GetRealInitValue("fm_thickness",0.0);
  field_like_ratio = GetRealInitValue("field_like_ratio",0.0);

  // Default rule gives J=+x -> sigma=+y.
  String sigma_rule = GetStringInitValue("sigma_rule","z_cross_j");
  Oxs_ToLower(sigma_rule);
  if(sigma_rule.compare("z_cross_j")==0) {
    sigma_sign = 1.0;   // sigma = z x J = (-Jy, Jx, 0)/|J|
  } else if(sigma_rule.compare("j_cross_z")==0) {
    sigma_sign = -1.0;  // sigma = J x z = ( Jy,-Jx, 0)/|J|
  } else {
    throw Oxs_ExtError(this,"Invalid initialization detected:"
                         " sigma_rule must be z_cross_j or j_cross_z");
  }

  // COMSOL current-density field, in A/m^2. Prefer "J_field";
  // "J" is accepted as a shorthand.
  if(HasInitValue("J_field")) {
    OXS_GET_INIT_EXT_OBJECT("J_field",Oxs_VectorField,J_init);
  } else {
    OXS_GET_INIT_EXT_OBJECT("J",Oxs_VectorField,J_init);
  }

  // Stage-varying multiplier for J.
  ...
  // SOT computes sigma from J_field, so mp propagation is disabled.
  mp_propagate = 0;
  fourpt_derivative = 0;
  ...
  if(method.compare("rk2")==0) {
    rkstep_ptr = &Oxs_SOTEvolve::TakeRungeKuttaStep2;
  } else if(method.compare("rk2heun")==0) {
    rkstep_ptr = &Oxs_SOTEvolve::TakeRungeKuttaStep2Heun;
  } else if(method.compare("rk4")==0) {
    rkstep_ptr = &Oxs_SOTEvolve::TakeRungeKuttaStep4;
  } else if(method.compare("rkf54m")==0) {
    rkstep_ptr = &Oxs_SOTEvolve::TakeRungeKuttaFehlbergStep54M;
  } else if(method.compare("rkf54s")==0) {
    rkstep_ptr = &Oxs_SOTEvolve::TakeRungeKuttaFehlbergStep54S;
  } else if(method.compare("rkf54")==0) {
    rkstep_ptr = &Oxs_SOTEvolve::TakeRungeKuttaFehlbergStep54;
  } else {
    throw Oxs_ExtError(this,"Invalid initialization detected:"
  ...
  max_dm_dt_output.Setup(this,InstanceName(),"Max dm/dt","deg/ns",0,
     &Oxs_SOTEvolve::UpdateDerivedOutputs);
  ...
  ave_J_output.Setup(this,InstanceName(),"average J","A/m^2",0,
     &Oxs_SOTEvolve::UpdateSpinTorqueOutputs);
  dm_dt_output.Setup(this,InstanceName(),"dm/dt","rad/s",1,
     &Oxs_SOTEvolve::UpdateDerivedOutputs);
  mxH_output.Setup(this,InstanceName(),"mxH","A/m",1,
     &Oxs_SOTEvolve::UpdateDerivedOutputs);
  precession_output.Setup(this,InstanceName(),"Precession term","rad/s",0,
     &Oxs_SOTEvolve::UpdateSpinTorqueOutputs);
  damping_output.Setup(this,InstanceName(),"Damping term","rad/s",0,
     &Oxs_SOTEvolve::UpdateSpinTorqueOutputs);
  dl_sot_output.Setup(this,InstanceName(),"DL SOT term","rad/s",0,
     &Oxs_SOTEvolve::UpdateSpinTorqueOutputs);
  fl_sot_output.Setup(this,InstanceName(),"FL SOT term","rad/s",0,
     &Oxs_SOTEvolve::UpdateSpinTorqueOutputs);
  spin_torque_output.Setup(this,InstanceName(),"SOT torque","rad/s",0,
     &Oxs_SOTEvolve::UpdateSpinTorqueOutputs);
  Jmp_output.Setup(this,InstanceName(),"J*sigma","A/m^2",0,
     &Oxs_SOTEvolve::UpdateSpinTorqueOutputs);
  ...
  precession_output.Register(director,-5);
  damping_output.Register(director,-5);
  dl_sot_output.Register(director,-5);
  fl_sot_output.Register(director,-5);
...
void Oxs_SOTEvolve::UpdateMeshArrays(const Oxs_Mesh* mesh)
{
  ...
  if(rmesh==NULL) {
    String msg="Import mesh to Oxs_SOTEvolve::UpdateMeshArrays"
      " is not an Oxs_RectangularMesh object.";
    throw Oxs_ExtError(msg.c_str());
  }

  OC_REAL8m local_fm_thickness = fm_thickness;
  if(local_fm_thickness<=0.0) {
    local_fm_thickness = rmesh->DimZ() * rmesh->EdgeLengthZ();
  }
  if(local_fm_thickness<=0.0) {
    throw Oxs_ExtError(this,"Invalid initialization detected:"
                         " fm_thickness must be positive");
  }

  const OC_REAL8m hbar = 1.0545717e-34;
  const OC_REAL8m e_charge = 1.6021765e-19;
  const OC_REAL8m sot_prefactor
    = hbar * theta_SH / (2.0 * MU0 * e_charge * local_fm_thickness);

  // Temporarily store J_field in mp, then overwrite mp with sigma(r).
  J_init->FillMeshValue(mesh,mp);

  A.AdjustSize(mesh);
  B.AdjustSize(mesh);
  beta_q_plus.AdjustSize(mesh);
  beta_q_minus.AdjustSize(mesh);
  beta_eps_prime.AdjustSize(mesh);

  const OC_INDEX size = mesh->Size();
  OC_INDEX i,j;
  ave_J = 0.0;
  for(i=0;i<size;i++) {
    const OC_REAL8m Jx = mp[i].x;
    const OC_REAL8m Jy = mp[i].y;
    const OC_REAL8m Jmag_sq = Jx*Jx + Jy*Jy;

    // Collapse the original Slonczewski angular factor to 1.
    A[i] = 1.0;
    B[i] = 0.0;
    beta_q_minus[i] = 0.0;
    beta_eps_prime[i] = 0.0;

    if(Jmag_sq>0.0) {
      const OC_REAL8m Jmag = sqrt(Jmag_sq);
      ave_J += Jmag;

      // Local Spin Hall polarization. Default sigma_rule z_cross_j gives
      // J=+x -> sigma=+y.
      mp[i].Set(sigma_sign*(-Jy)/Jmag, sigma_sign*Jx/Jmag, 0.0);

      // beta_sot = hbar theta_SH |J| / (2 e mu0 t_F). Ms^{-1} and any
      // time-dependent J_profile multiplier are applied in Calculate_dm_dt.
      beta_q_plus[i] = sot_prefactor * Jmag;
      beta_eps_prime[i] = field_like_ratio * beta_q_plus[i];
    } else {
      mp[i].Set(0.0,0.0,0.0);
      beta_q_plus[i] = 0.0;
    }
  }
  if(size>0) ave_J /= size;

  // Zero SOT on fixed spins
 ...
 const Oxs_MeshValue<ThreeVector>& spin)
{
  ...
  if(!mp.CheckMesh(mesh)) {
    throw Oxs_ExtError("mp array not properly initialized"
                         " in Oxs_SOTEvolve::Propagate_mp");
  }

  const Oxs_RectangularMesh* rmesh
    = dynamic_cast<const Oxs_RectangularMesh*>(mesh);
  if(rmesh==NULL) {
    String msg="Import mesh to Oxs_SOTEvolve::Propagate_mp"
      " is not an Oxs_RectangularMesh object.";
    throw Oxs_ExtError(msg.c_str());
  }
...
OC_REAL8m Oxs_SOTEvolve::EvaluateJProfileScript
(OC_UINT4m stage,
...
OC_REAL8m
Oxs_SOTEvolve::PositiveTimestepBound
(OC_REAL8m max_dm_dt)
...
void Oxs_SOTEvolve::Calculate_dm_dt
...
void Oxs_SOTEvolve::CheckCache(const Oxs_SimState& cstate)
{
  ...
  if(!cache_good) {
    throw Oxs_ExtError(this,
       "Oxs_SOTEvolve::CheckCache: Invalid data cache.");
  }
}


void
Oxs_SOTEvolve::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
  ...
  // to Oxs_SOTEvolve::Step() takes 2 half-steps, and it is the
  ...
  // in by the Oxs_SOTEvolve::NegotiateTimeStep() method.
}


void Oxs_SOTEvolve::UpdateTimeFields
...
void Oxs_SOTEvolve::NegotiateTimeStep
...
{ // This routine negotiates with driver over the proper step size.
  ...
  if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
    throw Oxs_ExtError(this,
       "Oxs_SOTEvolve::NegotiateTimeStep: max_dm_dt not cached.");
  }
  ...
  if(!cstate.GetDerivedData("Timestep lower bound",
                            timestep_lower_bound)) {
    throw Oxs_ExtError(this,
       "Oxs_SOTEvolve::NegotiateTimeStep: "
...
void Oxs_SOTEvolve::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
    ...
    throw Oxs_ExtError(this,
                         "Oxs_SOTEvolve::TakeRungeKuttaStep2:"
...
void Oxs_SOTEvolve::TakeRungeKuttaStep2Heun
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
     ...
     !endstate->AddDerivedData("dE/dt",dE_dt)) {
    throw Oxs_ExtError(this,
                         "Oxs_SOTEvolve::TakeRungeKuttaStep2Heun:"
                         " Programming error; data cache already set.");
  }
...
void Oxs_SOTEvolve::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SOTEvolve::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{ // Runge-Kutta-Fehlberg routine with combined 4th and 5th
  ...
  default:
    throw Oxs_ExtError(this,
                 "Oxs_SOTEvolve::RungeKuttaFehlbergBase54:"
...
void Oxs_SOTEvolve::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SOTEvolve::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SOTEvolve::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
OC_REAL8m Oxs_SOTEvolve::MaxDiff
...
void Oxs_SOTEvolve::AdjustStepHeadroom(OC_INT4m step_reject)
...
OC_BOOL
Oxs_SOTEvolve::Step(const Oxs_TimeDriver* driver,
                      Oxs_ConstKey<Oxs_SimState> current_state_key,
                      ...
                      Oxs_Key<Oxs_SimState>& next_state_key)
{
  ...
  if(next_timestep<=0.0 ||
    ...
    if(cstate.stage_number==0
      ...
      throw Oxs_ExtError(this,
           "Oxs_SOTEvolve::Step; Programming error:"
  ...
  // -mjd, 21-July-2004
  {
    ...
    if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
      throw Oxs_ExtError(this,"Oxs_SOTEvolve::Step:"
...
void Oxs_SOTEvolve::UpdateDerivedOutputs(const Oxs_SimState& state)
{ // This routine fills all the Oxs_SOTEvolve Oxs_ScalarOutput's to
...
void Oxs_SOTEvolve::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
  ...
  const OC_BOOL precession_output_requested
    = (precession_output.GetCacheRequestCount()>0);
  const OC_BOOL damping_output_requested
    = (damping_output.GetCacheRequestCount()>0);
  const OC_BOOL dl_sot_output_requested
    = (dl_sot_output.GetCacheRequestCount()>0);
  const OC_BOOL fl_sot_output_requested
    = (fl_sot_output.GetCacheRequestCount()>0);

  if(precession_output_requested || damping_output_requested
     || dl_sot_output_requested || fl_sot_output_requested) {

    Oxs_MeshValue<ThreeVector>* precession = 0;
    Oxs_MeshValue<ThreeVector>* damping = 0;
    Oxs_MeshValue<ThreeVector>* dl_sot = 0;
    Oxs_MeshValue<ThreeVector>* fl_sot = 0;

    if(precession_output_requested) {
      precession_output.cache.state_id = 0;
      precession = &precession_output.cache.value;
      precession->AdjustSize(mesh);
    }
    if(damping_output_requested) {
      damping_output.cache.state_id = 0;
      damping = &damping_output.cache.value;
      damping->AdjustSize(mesh);
    }
    if(dl_sot_output_requested) {
      dl_sot_output.cache.state_id = 0;
      dl_sot = &dl_sot_output.cache.value;
      dl_sot->AdjustSize(mesh);
    }
    if(fl_sot_output_requested) {
      fl_sot_output.cache.state_id = 0;
      fl_sot = &fl_sot_output.cache.value;
      fl_sot->AdjustSize(mesh);
    }

    Oxs_MeshValue<ThreeVector> local_mxH;
    const Oxs_MeshValue<ThreeVector>* mxH_ptr = 0;
    if(mxH_output.cache.state_id == state.Id()) {
      mxH_ptr = &mxH_output.cache.value;
    } else {
      OC_REAL8m pE_pt;
      GetEnergyDensity(state,temp_energy,&local_mxH,NULL,pE_pt);
      mxH_ptr = &local_mxH;
    }
    ...
    for(i=0;i<size;i++) {
      if(Ms[i]==0) {
        if(precession) (*precession)[i].Set(0.0,0.0,0.0);
        if(damping)    (*damping)[i].Set(0.0,0.0,0.0);
        if(dl_sot)     (*dl_sot)[i].Set(0.0,0.0,0.0);
        if(fl_sot)     (*fl_sot)[i].Set(0.0,0.0,0.0);
      } else {
        const OC_REAL8m cell_alpha = alpha[i];
        const OC_REAL8m cell_mgamma = -1*gamma[i]; // gamma[i] stores gamma_LL

        const ThreeVector m = spin[i];
        const ThreeVector hterm = mxH[i]; // m x H_eff

        if(precession) {
          if(do_precess) {
            (*precession)[i] = hterm;
            (*precession)[i] *= -1*cell_mgamma;
          } else {
            (*precession)[i].Set(0.0,0.0,0.0);
          }
        }

        if(damping) {
          (*damping)[i] = hterm;
          (*damping)[i] *= cell_alpha;
          (*damping)[i] ^= m;
          (*damping)[i] *= cell_mgamma;
        }

        if(dl_sot || fl_sot) {
          ThreeVector p = mp[i];
          ThreeVector mxp = m; mxp ^= p; // m x sigma
          ...
          ThreeVector dlvec = mxp; dlvec ^= m; // m x (sigma x m)

          if(dl_sot) {
            (*dl_sot)[i].Set(0.0,0.0,0.0);
            if(do_precess) {
              ThreeVector tmp = dlvec;
              tmp *= betaeps;
              (*dl_sot)[i] += tmp;
            }
            ThreeVector tmp = mxp;
            tmp *= cell_alpha * betaeps;
            (*dl_sot)[i] += tmp;
            (*dl_sot)[i] *= cell_mgamma;
          }

          if(fl_sot) {
            (*fl_sot)[i].Set(0.0,0.0,0.0);
            ThreeVector tmp = dlvec;
            tmp *= cell_alpha * betaepsp;
            (*fl_sot)[i] += tmp;
            if(do_precess) {
              tmp = mxp;
              tmp *= -1*betaepsp;
    ...
    const OC_INDEX fixed_count = GetFixedSpinCount();
    for(j=0;j<fixed_count;j++) {
      const OC_INDEX it = GetFixedSpin(j);
      if(precession) (*precession)[it].Set(0.0,0.0,0.0);
      if(damping)    (*damping)[it].Set(0.0,0.0,0.0);
      if(dl_sot)     (*dl_sot)[it].Set(0.0,0.0,0.0);
      if(fl_sot)     (*fl_sot)[it].Set(0.0,0.0,0.0);
    }

    if(precession_output_requested) precession_output.cache.state_id = state.Id();
    if(damping_output_requested)    damping_output.cache.state_id = state.Id();
    if(dl_sot_output_requested)     dl_sot_output.cache.state_id = state.Id();
    if(fl_sot_output_requested)     fl_sot_output.cache.state_id = state.Id();
  }

  if(Jmp_output.GetCacheRequestCount()>0) {
    Jmp_output.cache.state_id = 0;

    Oxs_MeshValue<ThreeVector> jvec;
    J_init->FillMeshValue(mesh,jvec);
    ...
    for(i=0;i<size;i++) {
      if(Ms[i]==0) {
        ...
        const OC_REAL8m Jx = jvec[i].x;
        const OC_REAL8m Jy = jvec[i].y;
        const OC_REAL8m Jmag = sqrt(Jx*Jx + Jy*Jy);
        jmp[i] = mp[i];
        jmp[i] *= Jmult*Jmag;
      }
    }
